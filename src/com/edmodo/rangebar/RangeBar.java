...
public class RangeBar extends View {
    ...
    private static final int DEFAULT_INVERSION = 1;
    ...
    private int mTickStep = DEFAULT_TICK_STEP;
    ...
    private boolean mInversionOk = DEFAULT_INVERSION == 1;
    ...
    private boolean mStartSame = false;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putInt("TICK_STEP", mTickStep);
        bundle.putBoolean("INVERSION", mInversionOk);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            mTickStep = bundle.getInt("TICK_STEP");
            mInversionOk = bundle.getBoolean("INVERSION");
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount, mTickStep, mTickHeightDP, mBarWeight, mBarColor);
    ...
    private void rangeBarInit(Context context, AttributeSet attrs)
    {
        ...
        try {
            ...
            mTickStep = ta.getInteger(R.styleable.RangeBar_tickStep, DEFAULT_TICK_STEP);
            mTickHeightDP = ta.getDimension(R.styleable.RangeBar_tickHeight, DEFAULT_TICK_HEIGHT_DP);
            mInversionOk = ta.getInteger(R.styleable.RangeBar_inversion, DEFAULT_INVERSION) == 1;
    ...
    private void onActionDown(float x, float y) {
        ...
        } else if (!mRightThumb.isPressed() && mRightThumb.isInTargetZone(x, y)) {
        ...
        if (mListener != null) {
            mListener.onActionDownListener(this, mLeftIndex, mRightIndex);
        }
    }
    ...
    private void onActionUp() {
        ...
        if (mListener != null) {
            mListener.onActionUpListener(this, mLeftIndex, mRightIndex);
        }
    }
    ...
    private void onActionMove(float x) {
        ...
        if (mLeftThumb.isPressed()) {
        	if (mStartSame) {
        		// the two were at the same spot
        		if (mLeftThumb.getX() < x) {
        			// moving right, only move mRightThumb
        			releaseThumb(mLeftThumb);
        			pressThumb(mRightThumb);
        	...
        	if (mStartSame) {
        		// the two were at the same spot
        		if (mRightThumb.getX() > x) {
        			// moving left, only move mLeftThumb
        			releaseThumb(mRightThumb);
        			pressThumb(mLeftThumb);
        			moveThumb(mLeftThumb, x);
        		}        			
        ...
        if (mInversionOk) {
        	...
        	if (mLeftThumb.isPressed() && mLeftThumb.getX() > mRightThumb.getX()) {
                moveThumb(mRightThumb, mLeftThumb.getX()); // move both to mLeftThumb
        	}
        	else if (mRightThumb.isPressed() && mRightThumb.getX() < mLeftThumb.getX()) {
        		moveThumb(mLeftThumb, mRightThumb.getX()); // move both to mRightThumb
        	}
        }
    ...
    public static interface OnRangeBarChangeListener {
        ...
        public void onActionUpListener(RangeBar rangeBar, int leftThumbIndex, int rightThumbIndex);
        public void onActionDownListener(RangeBar rangeBar, int leftThumbIndex, int rightThumbIndex);
    }
}
